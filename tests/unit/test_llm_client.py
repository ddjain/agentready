...
def test_create_anthropic_client_uses_api_key_when_vertex_disabled():
    """When CLAUDE_CODE_USE_VERTEX is not set, returns Anthropic client with api_key."""
    with patch.dict(os.environ, {"CLAUDE_CODE_USE_VERTEX": ""}, clear=False):
...
def test_create_anthropic_client_raises_when_no_api_key_and_not_vertex():
    ...
    with patch.dict(os.environ, {"CLAUDE_CODE_USE_VERTEX": "0"}, clear=False):
        with pytest.raises(ValueError, match="ANTHROPIC_API_KEY required"):
            create_anthropic_client(api_key=None)


def test_create_anthropic_client_returns_vertex_when_configured():
    """When CLAUDE_CODE_USE_VERTEX=1 and Vertex env vars set, returns AnthropicVertex."""
    with patch.dict(
        os.environ,
        {
...
def test_create_anthropic_client_vertex_raises_without_project_id():
    """When CLAUDE_CODE_USE_VERTEX=1 but ANTHROPIC_VERTEX_PROJECT_ID missing, raises."""
...
def test_create_anthropic_client_vertex_raises_without_region():
    """When CLAUDE_CODE_USE_VERTEX=1 but CLOUD_ML_REGION missing, raises."""
    env = {"CLAUDE_CODE_USE_VERTEX": "1", "ANTHROPIC_VERTEX_PROJECT_ID": "my-project"}
    with patch.dict(os.environ, env, clear=False):
...
def test_create_anthropic_client_vertex_ignores_whitespace_in_use_vertex():
    """CLAUDE_CODE_USE_VERTEX is stripped; only '1' enables Vertex."""
    with patch.dict(
        os.environ,
        {
            "CLAUDE_CODE_USE_VERTEX": "  1  ",
