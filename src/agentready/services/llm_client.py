...
def create_anthropic_client(api_key: str | None = None) -> Anthropic | AnthropicVertex:
    ...
    When CLAUDE_CODE_USE_VERTEX=1: requires ANTHROPIC_VERTEX_PROJECT_ID and
    ...
    if use_vertex == "1":
        ...
        if not project_id:
            raise ValueError(
                "ANTHROPIC_VERTEX_PROJECT_ID env var required when CLAUDE_CODE_USE_VERTEX=1"
            )
        if not region:
            raise ValueError(
                "CLOUD_ML_REGION env var required when CLAUDE_CODE_USE_VERTEX=1"
            )
